...
# -*- coding: utf-8 -*-
"""
AutoAttack Evaluation Script (Defensive Distillation — Teacher Model) — English-documented version
...
- Loads a trained Vision Mamba (Vim) checkpoint for the *teacher* model of Defensive Distillation.
- Infers Vim architecture parameters from the checkpoint state_dict if a full config is not available.
- Wraps the model with optional ImageNet normalization (mean/std) and a silent-forward wrapper.
- Loads a test (or valid) dataset using torchvision ImageFolder.
...
Notes for users
- Paths in CFG are project-specific; update CKPT_CANDIDATES / OUT_DIR / DATA_ROOT as needed.
- MAX_SAMPLES limits evaluation for speed (increase for full evaluation).
- The loader returns unnormalized tensors; if USE_IMAGENET_NORM=True, NormalizeWrapper applies
  ImageNet normalization inside the model forward pass.

High-level pseudocode
1) Build DataLoader (test/valid) → collect all samples → (x_all, y_all)
2) Load checkpoint → extract state_dict → build Vim (infer params if needed) → load weights
3) Optionally wrap with NormalizeWrapper(mean/std) → SilentForwardWrapper
...
# =========================================================
# CONFIG (same epsilon list and AutoAttack goal; accelerated via small sample + single-tensor evaluation)
# =========================================================
class CFG:
    ...
    # Disable extra visuals for speed (still saves CSV + plots)
    SAVE_VIS = False
