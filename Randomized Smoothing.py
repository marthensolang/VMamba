...
def extract_state_dict(ckpt: Dict[str, Any]) -> Dict[str, torch.Tensor]:
    ...
    raise KeyError("Tidak menemukan state_dict. Cek format checkpoint Anda.")
...
def parse_class_mapping(classmap_obj: Any) -> Tuple[List[str], Dict[str, int], Dict[int, str]]:
    ...
    - {"idx_to_class": {...}} atau {"id2label": {...}} (dict/list)
    - {"class_to_idx": {...}} atau {"label2id": {...}}
    - dict langsung idx->class atau class->idx
    - nested {"mapping": {...}}, dst
    """
    # 1) list[str]
    ...
    if isinstance(classmap_obj, dict):
        ...
        raise ValueError(
            "Format class_mapping.json tidak dikenali. "
            f"Tipe={type(classmap_obj).__name__}, keys(sample)={keys}, sample_kv={sample_kv}"
        )

    raise ValueError(f"Format class_mapping.json tidak dikenali. Tipe={type(classmap_obj).__name__}")
...
class FolderDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        try:
            ...
            print(f"[WARN] gagal load {img_path}: {e}")
...
def create_data_loaders(classes: List[str], img_size: Tuple[int,int], device_is_cuda: bool):
    ...
    if VAL_DIR.exists():
        ...
        print("[INFO] VAL_DIR tidak ada, pakai TEST_DIR untuk validasi.")
        val_ds = FolderDataset(TEST_DIR, classes=classes, transform=val_transform, img_size=img_size)
    else:
        raise FileNotFoundError("Tidak ada valid/ atau test/ untuk validasi.")
...
def pick_vim_kwargs(config_obj: Any, ckpt_obj: Any) -> Dict[str, Any]:
    ...
    raise RuntimeError(
        "Tidak menemukan vim_kwargs di checkpoint/config. "
        "Pastikan training base menyimpan dict 'vim_kwargs'."
    )
...
def load_vim_from_checkpoint(
    ...
    if classes is None:
        print("[WARN] Tidak bisa parse class mapping dari file/config/ckpt. Fallback: scan folder train/ (berisiko mismatch label).")
        classes = sorted([p.name for p in TRAIN_DIR.iterdir() if p.is_dir()])

    if not classes or not all(isinstance(c, str) for c in classes):
        raise RuntimeError("classes tidak valid setelah parsing/fallback.")
    ...
    # Jangan inject arg baru seperti 'in_chans' (bikin error).
    # HANYA cek num_classes; kalau beda, pakai yang di vim_kwargs supaya head konsisten.
    if "num_classes" in vim_kwargs and vim_kwargs["num_classes"] != num_classes:
        print(
            ...
            "Untuk menjaga kompatibilitas head, dipakai nilai dari vim_kwargs."
        )
        num_classes = int(vim_kwargs["num_classes"])
    else:
        # jika tidak ada, pakai len(classes) tapi TIDAK menambah key baru (biar sama persis spt base).
        if "num_classes" not in vim_kwargs:
            print(f"[INFO] vim_kwargs tidak punya 'num_classes', pakai len(classes)={num_classes} (tidak menambahkan key baru).")

    try:
        from vision_mamba import Vim
    except Exception as e:
        raise ImportError(
            "Gagal import Vim. Pastikan modul `vision_mamba.py` atau package `vision_mamba` tersedia.\n"
            "Contoh: from vision_mamba import Vim"
        ) from e
    ...
    try:
        ...
        print("[ERROR] State_dict mismatch terdeteksi!")
        print("  missing_keys (contoh):", missing[:15])
        print("  unexpected_keys (contoh):", unexpected[:15])

        if bad_missing or bad_unexp:
            raise RuntimeError(
                "STOP agar tidak menghasilkan angka palsu: mismatch bukan hanya classifier/head. "
                "Periksa bahwa vim_kwargs benar-benar sama dengan training sebelumnya."
            ) from e

        print("[WARN] Mismatch hanya pada head/classifier. Melanjutkan load strict=False.")
