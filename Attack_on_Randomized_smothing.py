...
"""
TRUE Adaptive PGD Attack Evaluation — Randomized Smoothing (RS) Model
===================================================================

Purpose
-------
This script evaluates adversarial robustness of a *Randomized Smoothing* fine-tuned model
using a TRUE adaptive (defense-aware) PGD attack across multiple epsilon budgets.

Key outputs (saved under: <BASE_DIR>_adaptive_attack_complete_TRUE)
-------------------------------------------------------------------
- adaptive_attack_TRUE_results.csv
- robustness_curve_TRUE_rs_randomsmoothing.png
...
Assumptions
-----------
- Dataset folder structure: dataset_rambu_lalu_lintas/{train,valid,test}
- Inputs are normalized with ImageNet MEAN/STD before being fed to the model.
- EPS_PX_LIST is expressed in *pixel scale* (e.g., 2 means 2/255).

Why epsilon is converted to "normalized space"
----------------------------------------------
Because the input is normalized as: x_norm = (x - MEAN) / STD,
the L_infinity budget must be applied in the same space:
    eps_norm = eps / STD   (per-channel)

High-level pipeline (pseudocode)
--------------------------------
1) Resolve BASE_DIR and locate:
     - checkpoint (prefer final_certified_model.pth)
     - model config JSON (prefer vim_kwargs.json)
     - class mapping JSON (prefer classes.json)
   Create output folder: <BASE_DIR>_adaptive_attack_complete_TRUE

2) Load model & dataset safely
   - Instantiate Vision Mamba if available; otherwise fallback to a small CNN
   - Load checkpoint weights (strict=False, multiple key fallbacks)
   - Build ImageFolder(test) with transforms:
       Resize(256) -> CenterCrop(224) -> ToTensor() -> Normalize(MEAN, STD)
   - Optionally use a subset for ULTRA_LIGHT mode

3) Measure clean accuracy on the SAME subset used for the attack

4) For each epsilon in EPS_LIST:
   - Run RobustAdaptivePGDAttack with defense_type="rs":
       * EOT (Expectation over Transformation) by averaging logits over
         RS_EOT_SAMPLES Gaussian noise draws (sigma = RS_SIGMA)
   - Compute robust accuracy and save per-epsilon visual diagnostics

5) Save CSV report + robustness curve plot

Notes for reproducibility
-------------------------
- CUDA_LAUNCH_BLOCKING=1 is enabled for easier debugging (can be slower).
- Set ULTRA_LIGHT=False for a more thorough evaluation.
...
# ------------------------------------------------------------
# 0. KONFIGURASI & PATH (TARGET: RANDOMIZED SMOOTHING FINE-TUNING_new)
# (EN) Resolve model directory, pick checkpoint/config/class mapping, and create output folder.
# ------------------------------------------------------------
BASE_DIR = Path("Randomized Smoothing Fine-tuning_new")

# Periksa apakah direktori ada
if not BASE_DIR.exists():
    alternative_paths = [
        Path("RandomizedSmoothing"),
        Path("randomized_smoothing"),
        Path("RS"),
        Path("randomized-smoothing"),
        Path("outputs/randomized_smoothing"),
        Path("models/randomized_smoothing"),
    ]
    ...
    if not BASE_DIR.exists():
        ...
        for item in Path(".").iterdir():
            if item.is_dir():
                print(f"  - {item}")
        raise FileNotFoundError("Direktori Randomized Smoothing tidak ditemukan")
...
# Target files sesuai permintaan
PREFERRED_CKPT = BASE_DIR / "final_certified_model.pth"
FALLBACK_CKPT  = BASE_DIR / "checkpoint_epoch_15.pth"
PREFERRED_CFG  = BASE_DIR / "vim_kwargs.json"
PREFERRED_CLS  = BASE_DIR / "classes.json"
TRAIN_HIST_PATH = BASE_DIR / "training_history.json"  # tidak dipakai, hanya path referensi

# Cari file checkpoint (fallback scanning)
ckpt_patterns = [
...
# Tentukan CKPT_PATH (prioritas: final_certified_model.pth -> checkpoint_epoch_15.pth -> hasil scan)
if PREFERRED_CKPT.exists():
    ...
    raise FileNotFoundError(f"Tidak ada file checkpoint ditemukan di {BASE_DIR}")

print(f"✓ Menggunakan checkpoint: {CKPT_PATH}")

# Tentukan CONFIG_PATH (prioritas: vim_kwargs.json -> hasil scan json -> None)
...
# Tentukan CLASSMAP_PATH (prioritas: classes.json -> hasil scan json yang relevan -> dataset mapping -> None)
mapping_patterns = [
...
if PREFERRED_CLS.exists():
    CLASSMAP_PATH = PREFERRED_CLS
...
BASE_GLOBAL_CLEAN_ACC = 45.5  # % (ganti dengan akurasi model RS jika diketahui)

# Defense type: untuk model Randomized Smoothing
DEFENSE_TYPE = "rs"  # Ubah dari "plain" ke "rs" untuk Randomized Smoothing

RS_SIGMA = 0.25  # Sesuaikan dengan sigma yang digunakan pada pelatihan RS
RS_EOT_SAMPLES = 8
...
# (EN) Safe model instantiation + label range validation to prevent runtime errors.
# ------------------------------------------------------------
class SafeVimWrapper(nn.Module):
    """Wrapper yang aman + validasi label."""
...
# (EN) Defense-aware PGD: for RS uses EOT over Gaussian noise to approximate gradients.
# ------------------------------------------------------------
...
# (EN) Clean-accuracy baseline on the same evaluation subset.
# ------------------------------------------------------------
@torch.no_grad()
...
# (EN) Save diagnostics: progression curve, clean-vs-adv grid, perturbation heatmap.
# ------------------------------------------------------------
...
# (EN) Run TRUE adaptive PGD per epsilon and generate the visual artifacts.
# ------------------------------------------------------------
def evaluate_adaptive_attack_safe(
    ...
    if saved_examples is not None:
        ...
        grid_plot = visualizer.create_adversarial_examples_grid(clean_imgs, adv_imgs, clean_preds, adv_preds, targets, eps)
...
# ------------------------------------------------------------
# 6. LOAD MODEL & DATA (AMAN, PAKAI FILE RANDOMIZED SMOOTHING)
# (EN) Robust loading: supports multiple checkpoint formats and class-mapping JSON variants.
# ------------------------------------------------------------
def safe_load_model_and_data(device: torch.device) -> Tuple[nn.Module, DataLoader, Dict[int, str]]:
    print("🔍 Scanning for required files in Randomized Smoothing directory...")
    ...
    # Load class mapping (classes.json bisa LIST atau DICT)
    ...
    # ✅ FIX: handle list/dict untuk classes.json
    def build_idx_to_class(mapping) -> Dict[int, str]:
        if mapping is None:
            return {}

        # CASE 1: classes.json berupa LIST ["classA","classB",...]
        if isinstance(mapping, list):
            return {int(i): str(name) for i, name in enumerate(mapping)}

        # CASE 2: mapping berupa DICT
        if isinstance(mapping, dict):
            # format {"classes":[...]}
            if "classes" in mapping and isinstance(mapping["classes"], list):
                return {int(i): str(name) for i, name in enumerate(mapping["classes"])}

            if "idx_to_class" in mapping:
                sub = mapping["idx_to_class"]
                if isinstance(sub, list):
                    return {int(i): str(name) for i, name in enumerate(sub)}
                if isinstance(sub, dict):
                    return {int(k): str(v) for k, v in sub.items()}

            if "class_to_idx" in mapping and isinstance(mapping["class_to_idx"], dict):
                sub = mapping["class_to_idx"]
                return {int(v): str(k) for k, v in sub.items()}

            # fallback: dict langsung (key bisa "0","1", dst)
            all_keys_int = True
            ...
            # fallback terakhir: coba dibalik (value -> key)
            try:
                return {int(v): str(k) for k, v in mapping.items()}
            except Exception:
                print("❌ Gagal membangun idx_to_class dari mapping dict. Contoh:", list(mapping.items())[:5])
                return {}
    ...
    try:
        ...
        print("✓ Checkpoint loaded successfully (fallback torch.load)")
    except Exception as e:
    ...
    # Merge dengan config dari file jika ada
    if cfg:
    ...
    if isinstance(ckpt, dict):
        for key in ["model", "model_state", "state_dict", "model_state_dict", "weights"]:
            if key in ckpt and isinstance(ckpt[key], dict):
                try:
                    incompat = model.model.load_state_dict(ckpt[key], strict=False)
                    loaded = True
                    print(f"✓ Loaded weights from '{key}'")
                    try:
                        if hasattr(incompat, "missing_keys") and incompat.missing_keys:
                            print(f"  Missing keys: {incompat.missing_keys}")
                        if hasattr(incompat, "unexpected_keys") and incompat.unexpected_keys:
    ...
    if not loaded:
        try:
            if isinstance(ckpt, dict):
                incompat = model.model.load_state_dict(ckpt, strict=False)
...
# (EN) Export CSV and plot robust accuracy vs epsilon (pixel scale).
# ------------------------------------------------------------
def create_visualizations(results: List[Dict], save_dir: Path):
    ...
    plt.axhline(y=df["clean_acc_percent"].iloc[0], linestyle="--", linewidth=2,
                label="Clean Accuracy (subset, RS model)")
    ...
    plt.ylabel("Accuracy (%)")
    plt.title(f"Robustness vs Epsilon (Adaptive PGD - {DEFENSE_TYPE.upper()} / Randomized Smoothing Model)")
    plt.grid(True, alpha=0.3)
    plt.legend()
    out_path = save_dir / f"robustness_curve_TRUE_{DEFENSE_TYPE}_randomsmoothing.png"
    plt.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close()
...
# ------------------------------------------------------------
# 8. MAIN
# (EN) End-to-end pipeline: load -> clean acc -> loop eps -> save CSV + plots.
# ------------------------------------------------------------
def main():
    ...
    print("Target model      : Randomized Smoothing Model")
    print(f"Model dir         : {BASE_DIR}")
    print(f"Checkpoint        : {CKPT_PATH.name}")
    print(f"Defense type      : {DEFENSE_TYPE} (Randomized Smoothing)")
    ...
    try:
        print("\n📥 Step 1: Loading Randomized Smoothing model and data...")
        ...
        print("\n🧪 Step 2: Clean accuracy baseline on same subset (Randomized Smoothing model)...")
        clean_acc = evaluate_clean_acc(model, eval_loader, device, MAX_SAMPLES)
        clean_acc_percent = clean_acc * 100.0
        print(f"   ✓ Clean Acc (subset, RS model): {clean_acc_percent:.2f}%")
        ...
        try:
            ...
            }, ADAPTIVE_DIR / "attack_configuration_TRUE_randomsmoothing.pth")
            print(f"   ✓ Configuration saved to: {ADAPTIVE_DIR / 'attack_configuration_TRUE_randomsmoothing.pth'}")
        except Exception as e:
            print(f"   ⚠ Could not save configuration: {e}")

        print("\n🎯 Step 4: Running TRUE Adaptive PGD on Randomized Smoothing model...")
        results = []
        ...
        df = pd.DataFrame(results)
        print("\n" + "=" * 70)
        print("SUMMARY (TRUE ADAPTIVE - RANDOMIZED SMOOTHING MODEL)")
        print("=" * 70)
        cols_show = [
            "epsilon_255",
            "clean_acc_percent",
