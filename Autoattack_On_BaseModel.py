...
# -*- coding: utf-8 -*-
"""
AutoAttack Evaluation Script (Base Model) — English-documented version

This file preserves the original execution logic and code structure.
Only documentation was added/translated to improve readability for an
international GitHub audience.

What this script does
- Loads a trained Vision Mamba (Vim) checkpoint (or infers architecture from the state_dict).
- Wraps the model with optional ImageNet normalization (mean/std) and a silent-forward wrapper.
- Loads a test/validation dataset using torchvision ImageFolder.
- Collects the selected samples into a single tensor (fast evaluation loop).
- Runs AutoAttack (L∞) for eps ∈ {0.5,1,2,3,4,5,6,7,8}/255 using the standard suite:
...
Notes for users
- Paths in CFG are project-specific; update BASE_DIR / OUT_DIR / DATA_ROOT to match your setup.
- MAX_SAMPLES limits evaluation for speed (set to None or larger for full evaluation).
- The attack is run on *unnormalized* tensors from the loader, while NormalizeWrapper applies
  ImageNet normalization inside the model forward pass (when enabled).

High-level pseudocode
1) Load dataset subset → (x_all, y_all)
2) Load checkpoint → state_dict → build Vim → load weights (strict=False with safety threshold)
3) Optional NormalizeWrapper(mean/std) → SilentForwardWrapper
4) Compute clean accuracy on x_all
5) For each epsilon:
...
# =========================================================
# CONFIG (same eps list and AutoAttack goal; accelerated via small sample + single-tensor evaluation)
# =========================================================
class CFG:
    # ====== ONLY the target model path section should be changed ======
    ...
    # AutoAttack standard suite (fixed)
    AA_VERSION = "standard"
    AA_ATTACKS = ["apgd-ce", "apgd-dlr", "fab-t", "square"]

    # Speed-up: small sample + single-tensor evaluation
    IMG_SIZE = 224
    ...
    # Disable extra visual saving for speed (still saves CSV + plots)
    SAVE_VIS = False

    # Prevent hanging: silence prints from internal model forward
...
def pick_existing(paths: List[Path]) -> Path:
    for p in paths:
        if p.exists():
            return p
    raise FileNotFoundError("No checkpoint found from CKPT_CANDIDATES. Please check your paths.")
...
def main():
    ...
    print(f"[CFG ] {cfg_path}" if cfg_path else "[CFG ] not found (OK, inferring from state_dict).")
    ...
    if miss + unex > CFG.STRICT_LOAD_THRESHOLD:
        print("[FATAL] Large mismatch between checkpoint keys and the inferred Vim architecture.")
        print("  example missing:", incompat.missing_keys[:10])
        print("  example unexpected:", incompat.unexpected_keys[:10])
        raise RuntimeError("STOP to avoid producing misleading numbers.")
    ...
    # Collect all samples into a single tensor (very fast epsilon loop).
    ...
    eps_pbar = tqdm(CFG.EPS_LIST, desc="AutoAttack over epsilons", mininterval=CFG.TQDM_MININTERVAL)
